Public Class Form1
    Private Class Ball
        Public x As Integer
        Public y As Integer
        Public tx As Integer
        Public ty As Integer
        Public vx As Integer
        Public vy As Integer
        Public Sub New(ByVal _x, ByVal _y)
            x = _x
            y = _y
        End Sub

        Public Sub Shot(ByVAl _x, ByVAl _y)
            tx = _x
            ty = _y
            vx = (tx - x) / 8
            vy = (ty - y) / 8
        End Sub

        Public Sub Move()
            If Math.Abs(tx - x) > 8 Then
                x = x + vx
            End If
            If Math.Abs(ty - y) > 8 Then
                y = y + vy
            End If
        End Sub
    End Class
    Private Class Player
        Public Sub New(ByVal num, ByVal _x, ByVal _y)
            x = _x
            y = _y
        End Sub
        Public Sub Move(Byref _ball As Ball, Byref _p As Player)
            Const mv As Integer = 2
            Dim r As Integer = Rnd() * 5
            Select Case r
                Case 0 
                    If Math.Abs(_ball.x - x) < 24 And Math.Abs(_ball.y - y) < 24 Then
                        _ball.Shot(_p.x,_p.y)
                    End If
                Case 1
                    x = x + mv
                Case 2
                    x = x - mv
                Case 3
                    y = y + mv
                Case 4
                    y = y - mv
            End Select
        End Sub
        Public num As Integer
        Public x As Integer
        Public y As Integer
        Public width As Integer = 32
        Public height As Integer = 32
    End Class
    Private bmp As Bitmap
    Private gnd As Bitmap
    Private sp1 As Bitmap
    Private sp2 As Bitmap
    Private bal As Bitmap
    Private bl As Ball
    Private Team(11) As Player
    Private Enem(11) As Player
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Dim _g As Graphics
        _g = Graphics.FromImage(bmp)
        _g.DrawImage(gnd, 0, 0)
        Dim r As Integer = Rnd() * 10
        For i = 0 To 10 Step 1
            If i = 0 Then
                r = 10
            Else
                r = i - 1
            End If
            Team(i).Move(bl, Team(r))
            _g.DrawImage(sp1, Team(i).x, Team(i).y)
        Next
        For i = 0 To 10 Step 1
            If i = 0 Then
                r = 10
            Else
                r = i - 1
            End If
            Enem(i).Move(bl, Enem(r))
            _g.DrawImage(sp2, Enem(i).x, Enem(i).y)
        Next
        bl.Move()
        _g.DrawImage(bal, bl.x, bl.y)
        _g.Dispose()
        PictureBox1.Image = bmp
    End Sub

    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        bmp = New Bitmap(PictureBox1.Width, PictureBox1.Height)
        gnd = New Bitmap("..\..\Ground.png")
        sp1 = New Bitmap("..\..\SoccorPlayerNara.png")
        sp2 = New Bitmap("..\..\SoccorPlayer2.png")
        bal = New Bitmap("..\..\Ball.png")
        Team(0) = New Player(0, 150, 100)
        Team(1) = New Player(1, 150, 200)
        Team(2) = New Player(2, 150, 300)
        Team(3) = New Player(3, 300, 100)
        Team(4) = New Player(4, 300, 200)
        Team(5) = New Player(5, 300, 300)
        Team(6) = New Player(6, 450, 100)
        Team(7) = New Player(7, 450, 200)
        Team(8) = New Player(8, 450, 300)
        Team(9) = New Player(9, 600, 100)
        Team(10) = New Player(10, 600, 200)
        Enem(0) = New Player(0, 550, 100)
        Enem(1) = New Player(1, 550, 200)
        Enem(2) = New Player(2, 550, 300)
        Enem(3) = New Player(3, 400, 100)
        Enem(4) = New Player(4, 400, 200)
        Enem(5) = New Player(5, 400, 300)
        Enem(6) = New Player(6, 250, 100)
        Enem(7) = New Player(7, 250, 200)
        Enem(8) = New Player(8, 250, 300)
        Enem(9) = New Player(9, 100, 100)
        Enem(10) = New Player(10, 600, 200)
        bl = New Ball(600, 200)
        Timer1.Start()
    End Sub
End Class
